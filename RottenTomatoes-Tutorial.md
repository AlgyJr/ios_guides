...
* The Storyboard (canvas) in Interface Builder (IB) will show a ViewController by default . We are going to replace this with a TableViewController, so delete it.
* If the IB library is not open press control-ALT-command-3 to open it. From the library, drag a TableViewController component into the Storyboard
...
* In IB, select the Cell ( it is easier to select the Cell in the Outline View to the left of the canvas), select theAttribute Inspector and set the "Style" box to "Custom". This tells Xcode that we are creating a custom cell, as opposed to one of the 4 standard ones.
* In addition, in the Attributes Inspector with the Cell selected, set the Identifier box to "MyMovieCell". We will use this identifier later inside our code to dynamically request IOS to create our custom cell and return a pointer to, so that we can load it with the movie content we want to display.
* From the IB library, drag a UIImageView and two UILabel controls inside the Cell. Place the UIImageView on the left hand side, one UILabel to the right side of the image to be used for the movie title, and one UILabel below to be used for the movie synopsis.
...
When the user clicks on a cell in the MoviesTableViewController, the "prepareForSegue:sender" method is called automatically. This method is called to help transition to the a next screen. Within this method one can pass data to the viewController about to be displayed, by storing data in that viewController's properties. The "segue" parameter of this method carries information about the Segue connector that is associated with the current transition. There could be multiple Segues in a design, thus by using the Segue identifier we can identify which Segue is being used. 
<br/> <br/>We will create a new class called "Movie" to store the information about the selected movie that we want to pass from the MoviesTableViewController to the MovieDetailViewController. To do this we need to create a property of class  Movie, call it "movieDetail",  in the MovieDetailViewController that will receive this information before the MovieDetailViewController is displayed. The place in the MoviesTableViewController where we pass a movie's data to the MovieDetailViewController is inside the  "prepareForSegue:sender" method, and we do it by storing the movie's information in the 'movieDetail' property of the MovieDetailViewController we created.   
